...
# ======================================================================
# BOT 80s — Proven 80.912s on Corkscrew (Phase 5 optimized)
# This is the KNOWN GOOD baseline. Do not modify this file.
# ======================================================================
...
# --- 6. Track-Specific Zones (Laguna Seca — 3 of max 4) ---
...
def calculate_steering(S, current_track):
    ...
    target_pos = S['trackPos'] * CENTERING_GAIN
...
def apply_abs(S, current_brake):
    ...
    if slip > ABS_SLIP_THRESHOLD:
        ...
        reduction = (ABS_KP * slip_error + ABS_KI * apply_abs.slip_integral + ABS_KD * d_slip)
...
def speed_control(S, R):
    ...
    if corner_warning < fwd_dist * LOOKAHEAD_BLEND_THRESHOLD and fwd_dist > 10:
        effective_dist = fwd_dist * LOOKAHEAD_WEIGHT + corner_warning * (1.0 - LOOKAHEAD_WEIGHT)
    else:
    ...
    if effective_dist == -1 or effective_dist > 200: effective_dist = 200.0
    elif effective_dist <= 0: effective_dist = 1.0
    ...
    if delta_dist > 0.5 and effective_dist > 30.0:
        ...
        target_speed -= abs(R['steer']) * MID_CORNER_PENALTY
    ...
    if speed_error > BRAKE_TRIGGER_MARGIN:
        ...
        brake = max(0.0, min(1.0, brake)); accel = 0.0
    elif speed_error > 0:
        brake = TRAIL_BRAKE_FACTOR * speed_error
        brake = max(0.0, min(1.0, brake)); accel = TRAIL_BRAKE_THROTTLE
    else:
        accel = 1.0
        if current_speed > target_speed - PARTIAL_THROTTLE_MARGIN: accel = PARTIAL_THROTTLE_VALUE
        brake = 0.0
    if current_speed < 10 and effective_dist > 10 and brake < 0.1: accel = 1.0; brake = 0.0
    if effective_dist >= 200: accel = 1.0; brake = 0.0
    return accel, brake
...
def traction_control(S, R):
    ...
    if slip > TCS_THRESHOLD:
        ...
        traction_control.slip_integral = max(0.0, min(traction_control.slip_integral, TCS_INTEGRAL_CAP))
        d_slip = slip_error - traction_control.prev_slip
        reduction = TCS_KP * slip_error + TCS_KI * traction_control.slip_integral + TCS_KD * d_slip
        accel -= reduction; accel = max(0.0, accel)
    else:
...
def drive(c, step_counter):
    ...
    if not drive.track_profiled:
        ...
        if not drive.track_profiled:
            ...
            if 30 <= step_counter <= 50: drive.fp2_hist.append(S['track'][9])
                ...
                matched_track = None
                for t_name, (efp1, efp2) in TRACK_FINGERPRINTS.items():
                    if efp1 < 0: continue
                    if abs(drive.fp1 - efp1) < FINGERPRINT_TOLERANCE and abs(drive.fp2 - efp2) < FINGERPRINT_TOLERANCE:
                        matched_track = t_name; break
                if matched_track: drive.current_track = matched_track
    ...
    if drive.current_track == "cs":
        ...
        if 3300 < current_dist < 3550:
            R['accel'] = 1.0; R['brake'] = 0.0

    R['gear'] = shift_stable(S, R, step_counter)
    if step_counter < 50:
        R['accel'] = 1.0; R['brake'] = 0.0; R['gear'] = 1; R['clutch'] = 0.0
    R['meta'] = 0
    return
