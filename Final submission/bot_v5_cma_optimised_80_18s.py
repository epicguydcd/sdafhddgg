...
# ======================================================================
# BOT — Deep CMA-ES optimized (80.182s standing lap)
# Based on v7-CMA 80.27s with ±3-5% deep refinement (500 trials)
# ======================================================================

# --- 1. Steering PD ---
STEER_KP = 8.399184854045727
STEER_KD = 10.360924840412178
CENTERING_GAIN = 0.026278308118437656

# --- 2. Speed Control ---
BASE_SPEED_MULT = 6.783990171198141
DISTANCE_SCALER = 13.642964473144765
CORNER_EXIT_PUNCH = 54.28812034215107
MID_CORNER_PENALTY = 5.371846169134437
WALL_FEAR_MULT = 3.2751166162632352
LOOKAHEAD_BLEND_THRESHOLD = 0.31903997750466984
LOOKAHEAD_WEIGHT = 0.8552954959238726
SPEED_PENALTY_REFERENCE = 150.27501272576808

# --- 3. Braking ---
BRAKE_KP = 0.1394193388087487
BRAKE_KD = 0.006388142485283601
BRAKE_TRIGGER_MARGIN = 6.0654391903924925
TRAIL_BRAKE_FACTOR = 0.3328589451502008
TRAIL_BRAKE_THROTTLE = 0.06162428686704036
PARTIAL_THROTTLE_MARGIN = 8.934465684146533
PARTIAL_THROTTLE_VALUE = 0.34851120801220753
...
R_S_L = 185.22698270457852
R_T_H_P = -0.6349772147835389
CORKSCREW_TURNIN_KP = 6.044983076395464
CORKSCREW_TURNIN_KD = 2.7389851442772617
T11S = 3205.479207020344
T11E = 3286.8866376283645
T11SL = 128.59096000368822
...
def speed_control(S, R, flying_lap=False):
    ...
    if flying_lap: ts *= 0.97  # 3% lower target speed on flying laps for stability
...
def drive(c, step_counter):
    ...
    if drive.current_track == "cs":
        # Flying laps need lower speed limits (higher approach speed)
        corkscrew_limit = R_S_L if not is_flying else R_S_L - 25.0
        t11_limit = T11SL if not is_flying else T11SL - 15.0
        # Pre-zone braking on flying laps (zone alone is too short at 220+ km/h)
        if current_lap > 0:
            pre_cork = R_S - 120
            if pre_cork < cd < R_S:
        ...
        if cd > T11E and S['distRaced'] > 500: R['accel'] = 1.0; R['brake'] = 0.0
    ...
    R['meta'] = 0
    # Telemetry: print key info every ~500m
