...
# ======================================================================
# BOT v3 — Proven 80.482s on Corkscrew (Phase 6 optimized)
# Features: PD steering, look-ahead, trail-braking, PID ABS/TCS,
#           out-in-out racing line, engine braking, speed-scaled penalty
# DO NOT MODIFY — this is a known-good checkpoint.
# ======================================================================

# --- 1. Steering PD ---
STEER_KP = 8.616023978509647
STEER_KD = 10.253216712881247
CENTERING_GAIN = 0.026278308118437656

# --- 2. Speed Control ---
BASE_SPEED_MULT = 6.752179590978522
DISTANCE_SCALER = 13.718775973419172
CORNER_EXIT_PUNCH = 51.40190501229689
MID_CORNER_PENALTY = 5.347371771934745
WALL_FEAR_MULT = 3.2751166162632352
LOOKAHEAD_BLEND_THRESHOLD = 0.32117116163132
LOOKAHEAD_WEIGHT = 0.8570481661379072
SPEED_PENALTY_REFERENCE = 150.27501272576808

# --- 3. Braking ---
BRAKE_KP = 0.14252306983126253
BRAKE_KD = 0.006388142485283601
BRAKE_TRIGGER_MARGIN = 6.394364845963751
TRAIL_BRAKE_FACTOR = 0.3198734861241366
...
def drive(c, step_counter):
    ...
    if drive.current_track == "cs":
        ...
        if T11S < cd < T11E:
            if S['speedX'] > T11SL: R['accel'] = 0.0; R['brake'] = 1.0
        # Full throttle to finish — race ends at line, no need to brake for T1
        if cd > 3300 and S['distRaced'] > 500: R['accel'] = 1.0; R['brake'] = 0.0
    R['gear'] = shift_stable(S, R, step_counter)
    if step_counter < 50: R['accel']=1.0; R['brake']=0.0; R['gear']=1; R['clutch']=0.0
    R['meta'] = 0
