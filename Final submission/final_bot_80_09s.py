...
def handle_off_track(S, R, step_counter):
    if not hasattr(handle_off_track, 'stuck_counter'):
        handle_off_track.stuck_counter = 0
        handle_off_track.reverse_counter = 0

    track_pos = S['trackPos']
    speed_ms = S['speedX'] / 3.6
    is_off_track = abs(track_pos) > 1.0

    if (abs(track_pos) > 0.8 and abs(speed_ms) < 1.0) or (is_off_track and S['speedX'] < 2.0):
        handle_off_track.stuck_counter += 1
    else:
        handle_off_track.stuck_counter = 0

    if handle_off_track.stuck_counter > 25:
        handle_off_track.reverse_counter = 50
        handle_off_track.stuck_counter = 0

    if handle_off_track.reverse_counter > 0:
        handle_off_track.reverse_counter -= 1
        R['gear'] = -1
        R['accel'] = 1.0
        R['brake'] = 0.0
        R['clutch'] = 0.0
        R['steer'] = -1.0 if track_pos > 0 else 1.0
        return True

    if is_off_track:
        target_angle = track_pos * 0.5 
        angle_error = S['angle'] - target_angle
        R['steer'] = max(-1.0, min(1.0, 3.0 * angle_error))
...
def drive(c, step_counter):
    ...
    is_recovering = handle_off_track(S, R, step_counter)
    ...
    if getattr(handle_off_track, 'reverse_counter', 0) <= 0:
    ...
    if step_counter < 15 and getattr(handle_off_track, 'reverse_counter', 0) <= 0: R['gear'] = 1
    R['meta'] = 0
